...
```json
{
  ...
  "core_conclusion": {
    ...
    "position_advice": {
      "no_position": "无持仓者：具体到价位的建议（如 '价格 5.10-5.15 区间分批建仓 10%，跌破 4.95 止损'）",
  ...
  "battle_plan": {
    ...
    "suggested_position": "建议仓位百分比（如 '5-15%' 或 '不超过 15%'）",
    "entry_plan": "建仓计划（如 '分 3 批：5.10/5.05/5.00 各 1/3'）",
    "risk_control": "风控要点（如 '严格止损，单股不超过总仓位 15%'）"
  },
...
{{
  ...
  "core_conclusion": {{
    ...
    "position_advice": {{
      "no_position": "无持仓者：具体到价位的建议（如 '5.10-5.15区间分批建仓10%，跌破4.95止损'）",
  ...
  "battle_plan": {{
    ...
    "stop_loss": "止损价+触发条件（如 '4.90，跌破前低+MA60'）",
    "take_profit": "止盈目标+触发条件（如 '5.50（短线）/ 5.80（中线）'）",
    "suggested_position": "建议仓位百分比（如 '5-15%' 或 '不超过15%'）",
    "entry_plan": "建仓计划（如 '分3批：5.10/5.05/5.00各1/3'）",
    "risk_control": "风控要点（如 '严格止损，单股不超过总仓位15%'）"
  }},
