...
@router.get("/quotes")
def get_quotes(symbols: str = Query(..., description="逗号分隔的股票代码")):
...
def get_news(symbol: str, count: int = Query(10, ge=1, le=30)):
    return market_service.get_stock_news(symbol, count=count)


@router.get("/hot")
def get_hot(top_n: int = Query(50, ge=5, le=5000)):
    """涨幅榜（同时包含全量数据，前端自行排序）"""
...
def cache_status():
    ...
    return {"total": len(all_q), "ready": len(all_q) >= 1000}


@router.get("/losers")
def get_losers(top_n: int = Query(15, ge=5, le=50)):
    """跌幅榜"""
    from apps.api.app.services.market_service import get_all_quotes_snapshot
    all_q = get_all_quotes_snapshot()
    losers = sorted(
...
@router.get("/active")
def get_most_active(top_n: int = Query(15, ge=5, le=50)):
    """成交额榜"""
    from apps.api.app.services.market_service import get_all_quotes_snapshot
    all_q = get_all_quotes_snapshot()
    active = sorted(
